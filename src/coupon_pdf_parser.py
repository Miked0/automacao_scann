...
class Coupon:
    raw_text: str
    numero: Optional[str] = None          # número canônico — preenchido com o 1º encontrado
    numero_sat: Optional[str] = None
    numero_coo: Optional[str] = None
    numero_nfce: Optional[str] = None
    source_filename: Optional[str] = None  # nome do arquivo PDF de origem
...
class CouponPDFParser:
    ...
    def __init__(self, pdf_pages: list[str], pdf_filename: Optional[str] = None):
        ...
        # Índices de busca
        self._index_numero: dict[str, Coupon] = {}  # campo numero canônico
    ...
    def _parse_block(self, block: str) -> Optional[Coupon]:
        ...
        # --- Extrai todos os números identificadores do bloco ---
        for m in _RE_NUMERO_CUPOM.finditer(block):
            groups = [g for g in m.groups() if g]  # primeiro grupo não-nulo
            if not groups:
                continue
            val = groups[0].strip()
            txt = m.group(0).lower()
            if "sat" in txt:
                ...
                # Número genérico — candidato ao campo canônico
                if not c.numero:
                    c.numero = val

        # numero canônico = SAT > NFC-e > COO > genérico
        if not c.numero:
            c.numero = c.numero_sat or c.numero_nfce or c.numero_coo

        # --- Enriquece com número do nome do arquivo ---
    ...
    def _register(self, c: Coupon) -> None:
        """Adiciona o cupom em todos os índices disponíveis."""
        if c.numero:
    ...
    def get_by_numero(self, numero: str) -> Optional["Coupon"]:
        """
        Busca principal: tenta todos os índices em ordem
        numero → sat → nfce → ecf.
        """
    ...
    def get_by_filename(self, filename: str) -> Optional["Coupon"]:
        """
        Busca pelo nome do arquivo PDF.
        Extrai o número do nome do arquivo e tenta localizar no índice.
        """
        stem = Path(filename).stem
        m = _RE_FILENAME_NUMERO.search(stem)
        if m:
            return self.get_by_numero(m.group(1))
        # Tenta o stem inteiro
        return self.get_by_numero(stem)

    def get_by_eans(self, eans: list[str]) -> Optional["Coupon"]:
        """Fallback: retorna o cupom que tem mais EANs em comum com a lista."""
