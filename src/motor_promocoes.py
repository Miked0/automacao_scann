# src/motor_promocoes.py
"""
Módulo 4 — MotorPromocoes  (v3 — L4: renomeado de promo_engine.py)
...
- validar_bin lê pagos[0].bin
- validar_pagamento compara inteiros (codigoTipoPago é int na API)
...
class MotorPromocoes:
    ...
    def validate(self, tipo, movimento, linha_roteiro):
        return self.validar(tipo, movimento, linha_roteiro)
    ...
    def _llevapaga(self, mov: dict, row: dict) -> ResultadoPromo:
        ...
        try:
            ...
            if trigger <= 0:
                return ResultadoPromo(ok=False, detalhe="trigger inválido para LLEVA_PAGA")
    ...
    def validar_pagamento(self, codigo_movimento: Any, tipo_roteiro: str) -> ResultadoPromo:
        ...
        codigo_movimento: valor de pagos[0].codigoTipoPago (inteiro)
        tipo_roteiro: texto do roteiro (ex: 'Dinheiro', 'Crédito', 'Débito', 'PIX')
        """
        try:
        ...
        roteiro_norm = str(tipo_roteiro).lower().strip()
        roteiro_norm = (
            roteiro_norm
    ...
    def validar_desconto_manual(
        ...
        if "sem desconto manual" in str(obs).lower():
            descuento = float(movimento.get("descuentoManual", 0))
            ok        = abs(descuento) <= TOLERANCIA
            return ResultadoPromo(
                ok=ok,
...
# Alias de compatibilidade — mantido para integrações externas
